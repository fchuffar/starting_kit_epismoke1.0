...
program <- function(data_train, data_test) {
    ...
    # # SIS
    # foo = apply(data_train[,-(1:4)], 2, function(cln) {
    #   # cln = data_train[,5]
    #   # boxplot(cln~data_train[,1])
    #   m = lm(cln~data_train[,1])
    #   res=c(beta_f=m$coefficients[[2]], beta_n=m$coefficients[[3]], pv=anova(m)[1,5])
    #   res
    # })
    # foo = t(foo)
    # foo = foo[order(foo[,3]),]
    # head(foo)
    # boxplot(cg25949550~smoking_status, data_train)
    #
    # layout(t(1:2), respect=TRUE)
    # plot(foo[,1], -log10(foo[,3]), col=(foo[,3]<0.0001)+1)
    # plot(foo[,2], -log10(foo[,3]), col=(foo[,3]<0.0001)+1)
    #
    # layout(t(1:2), respect=TRUE)
    # # use the best probe as predictor for smoking_status
    # probe = "cg25949550"
    # formula = as.formula(paste0("never01~", probe))
    # m = glm(formula, data_train, family=binomial(link = 'logit'))
    # pred = predict.glm(m, data_train, type="response")
    # boxplot(pred~data_train$never01)
    # pred = predict.glm(m, data_test, type="response")
    # data_test$never01 = ifelse(pred<0.33, 0, 1)
    #
    # formula = as.formula(paste0("current01~", probe))
    # m = glm(formula, data_train, family=binomial(link = 'logit'))
    # pred = predict.glm(m, data_train, type="response")
    # boxplot(pred~data_train$current01)
    # pred = predict.glm(m, data_test, type="response")
    # data_test$current01 = ifelse(pred<0.33, 0, 1)
    #
    # data_test$smoking_status = "former"
    # data_test[1==data_test$never01,]$smoking_status = "never"
    # data_test[1==data_test$current01,]$smoking_status = "current"
    #
    # head(data_test[,1:5], 30)
    #
    # data_pred = data_test$smoking_status
    # data_pred
